...
import { Plus, Pencil, Archive, ArchiveRestore, Settings as SettingsIcon, Layers, Timer, FlaskConical, RotateCcw, Save, Package, UserCircle2 } from "lucide-react";
...
interface Profile { id: string; nome: string; email: string; telefone: string | null; ativo: boolean }
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <Tabs defaultValue="permissoes" className="space-y-4">
        ...
        <TabsContent value="usuarios"><UsuariosSection /></TabsContent>
        <TabsContent value="linhas"><LinhasSection /></TabsContent>
        <TabsContent value="equipamentos"><EquipamentosSection /></TabsContent>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-start justify-between gap-3">
          <div>
            <CardTitle className="flex items-center gap-2">
              <UserCircle2 className="h-5 w-5 text-primary" /> Usuários
            </CardTitle>
            <CardDescription>Gerencie acessos, papéis e linhas atribuídas. Usuários nunca são apagados — apenas desativados.</CardDescription>
          </div>
...
function NovoUsuarioForm({ linhas, onSaved }: { linhas: Linha[]; onSaved: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (error || (data as any)?.error) {
      toast.error((data as any)?.error || error?.message || "Erro ao criar usuário"); return;
    }
...
function EditUserDialog({
  ...
  async function resetSenha() {
    ...
    setResetando(false);
    if (error || (data as any)?.error) {
      toast.error((data as any)?.error || error?.message || "Erro ao alterar senha"); return;
    }
    ...
    <Card>
      <CardHeader>
        <div className="flex items-start justify-between gap-3">
          <div>
            <CardTitle className="flex items-center gap-2">
              <Package className="h-5 w-5 text-primary" /> Equipamentos
            </CardTitle>
            <CardDescription>Catálogo de equipamentos por linha de produto</CardDescription>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <Badge variant="outline" className="mt-1 text-[10px]">{e.linhas_produto?.nome}</Badge>
                      {e.valor_referencia && (
            ...
            {items.length === 0 && (
              <p className="col-span-full text-sm text-muted-foreground">Nenhum equipamento cadastrado.</p>
            )}
          </div>
        )}
      </CardContent>
