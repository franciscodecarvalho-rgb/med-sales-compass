...
export default function MedicoDetail() {
  ...
  async function load() {
    ...
    const [m, u, a, ua, pp, esp, tk] = await Promise.all([
      ...
      supabase.from("medico_unidades").select("*, unidades_saude(id, nome, cidade, estado, status, medico_principal_id), papeis_contato(id, nome)").eq("medico_id", id),
...
function NovaTarefaMedicoDialog({ medicoId, userId, onCreated }: { medicoId: string; userId?: string; onCreated: () => void }) {
